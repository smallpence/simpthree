...
namespace SIMP
{
	...
	public class Image : IPictureRectangle
	{
		...
		{
			// prevent invalid widths & heights
			if (width <= 0) {
			...
			// centre is set to start in the middle of the image
		...
		public Color GetPixel(int x, int y) {
			return GetPixel(new FilePoint(x,y));
		}
		...
		/// Sets the colour of a pixel
		/// </summary>
		/// <param name="displayPoint">The coords of pixel to set</param>
		/// <param name="colour">The colour to set that pixel to</param>
		...
		public System.Drawing.Image GetDisplayImage(int width, int height, bool full) {
			...
			// adjusts the centre location if the image is odd
			...
			if (full) { // if the full image is requested
				...
				lastImage = newImage;
				return newImage;
			} else { // else if only changes since last display is requested
		...
		private void DrawChangedImage(Graphics GFX) {
			...
			foreach (FilePoint changedPixel in changedPixels) {
				...
				// if none of the layers mention that pixel
				if (!setPixel) {
					// make sure that its set to white - if the pixel has been erased
					// this is not needed when displaying the full image as the image starts as a blank canvas
					// but here the previous image is being edited so must make sure this is changed
					GFX.FillRectangle(
		...
		/// <summary>
		/// Change the zoomSettings centre location based on the value of the bar
		/// Used for when a bar is changed by user and needs this to be reflected in image
		/// </summary>
		/// <param name="newValue">Value bar has been changed to</param>
		/// <param name="axis">Axis of the bar</param>
		public void CentreFromBar(int newValue, EAxis axis) {
			int firstCrossPosition;
			switch (axis) {
				case EAxis.X:
					// check design docs for a more full explanation of the reasoning behind these sums
		...
		/// <summary>
		/// Determine what value the bar should have
		/// </summary>
		/// <param name="axis">Axis</param>
		/// <returns>Integer representing what to set the bar's value to</returns>
		...
		/// <summary>
		/// edits the filepoint such that it is in the image bounds
		/// </summary>
		...
		/// <summary>
		/// changes the centrelocation when the image size reduces
		/// this is so centre location is always somewhere on the image
		/// </summary>
