...
namespace SIMP
{
	...
	public partial class Workspace : Form
	{
		...
		public static Workspace OpenFile(FileStream stream) {
			...
			int width = (stream.ReadByte() * 256) + stream.ReadByte();
			int height = (stream.ReadByte() * 256) + stream.ReadByte();
			int layerCount = stream.ReadByte();
			
			Workspace newWorkspace = new Workspace(width,height);
			
			Image image = new Image(width,height,newWorkspace);
			...
			for (int i = 0; i < layerCount; i++) {
				...
				for (int x = 0; x < width; x++) {
					for (int y = 0; y < height; y++) {
						...
						G = SafeRead(stream);
						B = SafeRead(stream);
						newLayer.pixels[x,y] = new SolidBrush(Color.FromArgb(R,G,B));
					}
				}
